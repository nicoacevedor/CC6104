...
Primero, definimos la función `metropolis` la cual nos va a permitir calcular lo pedido
...
Antes de graficar, graficaremos la función de densidad gamma usando la función nativa de `R` `rgamma`. Para esto, generaremos $10^6$ elementos aleatorios considerando $\alpha = 5$ y $\beta = \frac{1}{5} = 0.2$
```{r}
# Distribución no normalizada
gamma_vect <- rgamma(1e6, shape = 5, rate = 0.2) * gamma(5)
hist(gamma_vect)
```
...
Notemos que ninguno se parece realmente a la distribución gamma. Algunas veces, para valores más altos de muestras, como $10^5$ o $10^6$ generan una forma un poco similar, pero realmente no es la distribución gamma.
